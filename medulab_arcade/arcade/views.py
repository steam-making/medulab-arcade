...
def future_career_video_save(request):
    ...
    if not birthdate:
        return JsonResponse({'success': False, 'error': '생년월일을 입력해 주세요.'}, status=400)
    if len(name) > 50:
        return JsonResponse({'success': False, 'error': '이름은 50자 이내로 입력해 주세요.'}, status=400)
    if len(birthdate) > 10:
        return JsonResponse({'success': False, 'error': '생년월일 형식이 올바르지 않습니다.'}, status=400)
    if not isinstance(data, dict):
        return JsonResponse({'success': False, 'error': '저장할 데이터 형식이 올바르지 않습니다.'}, status=400)

    existing = FutureCareerSave.objects.filter(name=name, birthdate=birthdate).first()
    ...
    obj, _ = FutureCareerSave.objects.update_or_create(name=name, birthdate=birthdate, defaults={'data': data})
...
def future_career_video_load(request):
    ...
    obj = FutureCareerSave.objects.filter(name=name, birthdate=birthdate).first()
    if not obj:
        return JsonResponse({'success': False, 'error': f'"{name}({birthdate})" 이름으로 저장된 내용이 없습니다.'})
...
def content_finder(request):
    ...
    if request.user.is_authenticated:
        try:
            draft_data = request.user.content_finder_draft.data
        except ContentFinderDraft.DoesNotExist:
            pass
...
@require_POST
def api_content_finder_draft_save(request):
    """나만의 콘텐츠 주제 찾기 - 회원 임시저장"""
    ...
    ContentFinderDraft.objects.update_or_create(user=request.user, defaults={'data': data})
...
def content_idea_finder(request):
    ...
    if request.user.is_authenticated:
        try:
            draft_data = request.user.content_idea_finder_draft.data
        except ContentIdeaFinderDraft.DoesNotExist:
            pass
...
@require_POST
def api_content_idea_finder_draft_save(request):
    """나만의 콘텐츠 아이디어 찾기 - 회원 임시저장"""
    if not request.user.is_authenticated:
        return JsonResponse({'ok': False, 'error': 'login required'}, status=401)
    try:
        data = json.loads(request.body)
        ...
        return JsonResponse({'ok': False, 'error': 'invalid json'}, status=400)
    ContentIdeaFinderDraft.objects.update_or_create(user=request.user, defaults={'data': data})
    return JsonResponse({'ok': True})
